...
This guide covers how to install the Glassify theme for Spotify using Spicetify.
...
1. Clone and install dependencies:
   ...
   ```bash
   git clone https://gitlab.com/sanoojes/spicetify-glassify.git
   cd spicetify-glassify
   bun install
   ```
...
3. Copy to Spicetify Themes directory:
   ...
   ```powershell [Windows]
   Copy-Item -Recurse dist "$env:APPDATA\spicetify\Themes\Glassify"
   ```

   ```bash [Linux/macOS]
   cp -r dist/ ~/.config/spicetify/Themes/Glassify
   ```
