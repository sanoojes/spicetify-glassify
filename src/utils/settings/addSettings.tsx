...
import { showModal } from '@app/utils/modal/showModal.tsx';
...
export function openModal() {
  ...
  showModal({ title: 'Glassify Settings', content: <Settings /> });
}

async function addSettingsEntry(openModal: () => void, position = 'menu') {
  if (
    position === 'nav' &&
    document.querySelector('.main-actionButtons button[aria-label="Glassify Settings"]')
  )
    return;
  ...
  if (position === 'menu') {
    ...
    settingsEntry = new Item('Glassify Settings', false, openModal, SETTINGS_ICON);
    settingsEntry.register();
  } else {
    settingsEntry = new Topbar.Button('Glassify Settings', SETTINGS_ICON, openModal, false, true);
  }
}
